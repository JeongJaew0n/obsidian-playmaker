...
작성 2026-09-18. **S1~S5 구현 완료(`3131d5d`).** 아래는 그 설계와 근거다.
...
브라우저에서 실물 `app.css` 와 추출한 CM6 baseTheme 위에 올려 측정했다.
플러그인이 실제로 생성하는 CSS 를 코드에서 뽑아 쓴 것이다.

| 항목 | 나 지금 해커 | 오래된 편지지 |
| --- | --- | --- |
| 뷰 배경 | `#080c08` | `#f2e8d5` |
| 읽기·편집 본문 | `#33ff66` | `#3a3226` |
| 헤딩 / 링크 / 코드 | `#b6ffcd` / `#7dffa8` / `#33ff66` | `#8a4a42` / `#4a6b85` / `#6b6a3f` |
| 글꼴 | 고정폭 | Iowan Old Style (세리프) |
| 인광 | `0 0 2.4px` | `none` |
| 줄높이 | 24px | 28px |
| 인용 | 보통 | 기울임 |
| 오버레이 | 스캔라인 | 종이 결(103°·11° 두 겹) |
| `pointer-events` | `none` | `none` |
| 200px 스크롤 후 오버레이 drift | 0 / 0 | 0 / 0 |
| 대조군 누출 | 없음 | 없음 |

마이그레이션도 확인했다.

- vault 의 실제 v1 `data.json` → v2. `preset: "amber"` 가 `variants.terminal: "amber"` 로 보존됨
- 경로·옵션이 있는 v1 → `notes` 에 `terminal` 로 매핑, 옵션은 `skinOptions.terminal` 로 이동
- `null` · 문자열 · 없는 스킨 id 같은 손상된 입력에도 예외 없이 기본값으로 떨어짐
- 어떤 경우에도 `applyToAllNotes` 는 `false`

**아직 확인하지 않은 것은 그대로다** — 실제 Obsidian 런타임에서의 동작(모드 전환·재시작 후
유지, 폴더 이름변경 추적), 밝은 바탕에서 Obsidian 기본 UI 요소가 어떻게 보이는지,
한글 본문 가독성.
